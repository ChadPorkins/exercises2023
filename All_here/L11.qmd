...
In this exercise, you will combine everything you've learned so far.

Do you remember Daniel from the first class with the GIS and ecology
companies? Every company has a website he manages (he is a very
hardworking guy). Let's help him write code that will give him a summary
of views on each web page.

Here are two pre-defined vectors: GIS and Ecology. They are the number
...
1.  If both GIS and Ecology are 30 or higher, set the Views equal to
    twice the sum of GIS and Ecology.

2.  If both GIS and Ecology are strictly below 20, set Views to equal
    half the sum of GIS and Ecology.

3.  In all other cases, set Views to equal GIS plus Ecology.
...
1.  Create a new loop that multiplies the value of q by 4 each time, the
    loop should run as long as q is less than 16, and after each
    multiplication, the following q should increase by 1.
...
R has a wide `apply()` function family, which consists of iterative
...
`sapply()` is a 'wrapper' function for `lapply()`, it returns a
...
You should get something like this:
 ...
 "Fahrenheit: 50 oF is equivalent to"  "Centigrade: 10 oC"
...
It is primarily used when the  dataset contains
...
The Ricker model is defined as: Where Nt is the population size at time
t, r is the population growth rate and K is the carrying capacity. Write
a function to simulate this model so you can conveniently determine the
effect of changing r and the initial population size N0. K is often set
to 100 by default, but you want the option of being able to change this
with your function. So, you will need a function with the following
